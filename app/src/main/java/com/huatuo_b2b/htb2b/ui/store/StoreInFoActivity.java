...
import com.huatuo_b2b.htb2b.adapter.StoreGoodsMyGridViewListAdapter;
...
import com.huatuo_b2b.htb2b.fragment.widget.RoundImageView;
...
public class StoreInFoActivity extends Activity implements OnGestureListener, OnTouchListener, OnClickListener {
    ...
    private RoundImageView storePic;

    private ImageView storeInFoPic;

    private ImageView favoritesAddID;        //店铺添加收藏按钮
    private ImageView favoritesDeleteID;    //店铺取消收藏按钮
...
//            	System.out.println("连续点击事件，共点击了 " + count + " 次");
    ...
    public void initViewID() {
        ...
        LinearLayout storeSearchID = (LinearLayout) findViewById(R.id.storeSearchID);

        storePic = (RoundImageView) findViewById(R.id.storePic);

        storeInFoPic = (ImageView) findViewById(R.id.storeInFoPic);

        favoritesAddID = (ImageView) findViewById(R.id.favoritesAddID);

        favoritesDeleteID = (ImageView) findViewById(R.id.favoritesDeleteID);
        ...
        goodsListViewAdapter = new StoreGoodsMyGridViewListAdapter(this, myApplication.getLoginKey(), myApplication.getMember_role());
...
//					Toast.makeText(StoreInFoActivity.this, getString(R.string.datas_loading_fail_prompt), Toast.LENGTH_SHORT).show();;
                }
            }
        });
    }
    ...
    public void initHeadImage(ArrayList<StoreInFo> list) {
        ...
        int screenWidth = UTILS.getScreenWidth(StoreInFoActivity.this);//屏幕宽度
        int m_height = screenWidth * Constants.AD_HEIGTH / Constants.AD_WHIDE;//广告图片的高度

        for (int i = 0; i < list.size(); i++) {
            StoreInFo bean = list.get(i);
